...
class TestJobModel:
    """Tests for Job model parsing."""

    def _make_detail_response(self, **core_overrides: object) -> dict:
        """Build a minimal JobScreenQuery-shaped response."""
        core = {
            ...
            "formatted_salary": "€50K - €55K",
            "team": "Platform",
            "job_description": "Build things",
            "job_page_url": "https://x.com/i/jobs/123",
        }
                    ...
                    "result": {
                        ...
                        "company_profile_results": {
                            "result": {
                                "rest_id": "c1",
                                "core": {"name": "Acme"},
                                "logo": {"normal_url": "https://logo.png"},
                            }
                        },
                        "user_results": {
                            "result": {
                                "core": {
                                    "screen_name": "recruiter",
                                    "name": "Recruiter",
                                },
                                "verification": {
                                    "verified": True,
    ...
    def test_detail_extracts_salary(self):
        """Salary fields must be extracted from job detail responses."""
        from clix.models.job import Job

        data = self._make_detail_response()
        job = Job.from_detail_result(data)
        assert job is not None
        assert job.salary_min == 50000
        assert job.salary_max == 55000
        assert job.salary_currency == "EUR"
        assert job.formatted_salary == "€50K - €55K"

    def test_detail_extracts_poster(self):
        """Poster fields must be extracted from job detail responses."""
        from clix.models.job import Job

        data = self._make_detail_response()
        job = Job.from_detail_result(data)
        assert job is not None
        assert job.poster_handle == "recruiter"
        assert job.poster_name == "Recruiter"
        assert job.poster_verified is True
        assert job.poster_verified_type == "Business"

    def test_detail_without_salary(self):
        """Jobs without salary data should return None/empty defaults."""
        from clix.models.job import Job
